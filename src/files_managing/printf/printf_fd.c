...
/*   Updated: 2025/10/08 01:55:43 by alpayet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_conversions_x_or_upperx_fd(char format_char, va_list args,
				int fd);
...
int	ft_putnbr_base_ul_fd(unsigned long nbr, char *base, int fd)
{
	...
	{
		quotient_printed = ft_putnbr_base_ul_fd(nbr / ft_strlen(base), base,
				fd);
		...
		return (ft_putchar_fd(base[nbr % ft_strlen(base)], fd));
	if (ft_putchar_fd(base[nbr % ft_strlen(base)], fd) == -1)
		return (-1);
	return (nbr_printed);
}

static int	ft_check_percent(char format_char, va_list args, int fd)
{
	if (format_char == 'c')
...
static int	ft_iterate_over_format(const char *format, va_list args, int fd)
{
	int	i;
	int	chars_printed;
	...
	while (format[i])
	{
		if (format[i] == '%')
		{
			check_var_printed = ft_check_percent(format[i + 1], args, fd);
...
int	ft_printf_fd(int fd, const char *format, ...)
{
	...
	chars_printed = ft_iterate_over_format(format, args, fd);
	va_end(args);
	return (chars_printed);
}
